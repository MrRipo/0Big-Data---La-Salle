...
mod.lm7 <- lm(count ~ poly(hour,2) + poly(season,2) + poly(weather,2)+
                 poly(temp,2) + poly(humidity,2) ,training_target)
summary(mod.lm7)

mod.lm7_step <- step(mod.lm7)
summary(mod.lm7_step)

# Colinealidad - https://www.rdocumentation.org/packages/regclass/versions/1.6/topics/VIF
vif(mod.lm1)
vif(mod.lm2) # We see humidity has more error
...
## REDO LM7 with each subset
#### LM7 5v (Poly) -> hour + season + weather + temp + humidity. Se incluyen terminos polinomicos de orden mayor
mod.lm7_work <- lm(count ~ poly(hour,2) + poly(season,2) + poly(weather,2)+
                poly(temp,2) + poly(humidity,2) ,training_target_work)
mod.lm7_free <- lm(count ~ poly(hour,2) + poly(season,2) + poly(weather,2)+
                     poly(temp,2) + poly(humidity,2) ,training_target_free)
summary(mod.lm7_work)
summary(mod.lm7_free)

mod.lm7_work_step <- step(mod.lm7_work)
mod.lm7_free_step <- step(mod.lm7_free)
summary(mod.lm7_work_step)
summary(mod.lm7_free_step)


#### Transformar datos horas por polinomio y hacer modelo lineal de workinday de 6 a 18h
# Coger los datos de workingday de 6h a 18h, que son los que se comportan como parábola
training_target_work_6to18 <- subset(training_target_work, hour >= 6 & hour <= 18)

# Ajustar a una función cuadrática (grado 2)
modelo_cuadratico <- lm(count ~ poly(hour,2) + poly(season,2) + poly(weather,2)+
                          poly(temp,2) + poly(humidity,2), training_target_work_6to18)
summary(modelo_cuadratico)

############### PREDICT #################
# Predicciones
testing$pred <- predict(mod.lm1, testing)
training_target$pred <- predict(mod.lm1, training_feat)

media_training_count = sum(training_target_work$count) /(dim(training_target_work$count)[1])
print(media_training_count)

training_target_free

scaterplot(training_target$holiday, training_target$count)

# Cálculo error cuadrático medio 
n <- dim(testing)[1]                                       # Tamaño muestral
EQM <- sum((pr-training$count)^2)/n                      # Error Cuadratico Medio
sqrt(EQM)                                               # Incertidumbre a posteriori                                                
sd(testing$Strength)                                       # Incertidumbre a priori --> El modelo me reduce a la mitad la incertidumbre en las predicciones
